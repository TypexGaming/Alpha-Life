...
if(!isNull _group) then {
	...
	_action = [
		format["Cette cachette est contrôlée par %1<br/>Etes-vous sûr que vous voulez prendre en charge leur zone de gang?",_gangName],
		"La cachette est actuellement sous contrôle ..",
		"Oui",
		"Non"
...
if(life_interrupted) exitWith {life_interrupted = false; titleText["Action annulé","PLAIN"]; life_action_inUse = false;_hideout setVariable["inCapture",false,true];};
life_action_inUse = false;

titleText["La cachette a été capturé.","PLAIN"];
_flagTexture = [
...
[[[0,1],format["%1 et son gang: %2 - ont pris le contrôle d'un repaire locale",name player,(group player) getVariable "gang_name" ]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
_hideout setVariable["inCapture",false,true];
_hideout setVariable["gangOwner",grpPlayer,true];
    // CREATE MARKER AT MAP BY Pictureclass
     
    _markername = str(getPos _hideout);
    _gangname2 = formatText["Capturer par: %1",(group player) getVariable "gang_name"];
