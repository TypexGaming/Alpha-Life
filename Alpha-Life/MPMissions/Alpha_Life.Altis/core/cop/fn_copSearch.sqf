...
if(count _invs > 0) then
{
	...
	[[0,format["%1 à %2$ de contrebande sur lui.",(_civ getVariable["realname",name _civ]),[_illegal] call life_fnc_numberText]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
}
	else
{
	_inv = "Pas d'objet illégale";
};
if(!alive _civ || player distance _civ > 5) exitWith {hint format["Couldn't search %1",_civ getVariable["realname",name _civ]]};
//hint format["%1",_this];
hint parseText format["<t color='#FF0000'><t size='2'>%1</t></t><br/><t color='#FFD700'><t size='1.5'><br/>Objets Illégaux</t></t><br/>%2<br/><br/><br/><br/><t color='#FF0000'>%3</t>"
,(_civ getVariable["realname",name _civ]),_inv,if(_robber) then {"Robbed the bank"} else {""}];

if(_robber) then
{
	[[0,format["%1 a été identifié comme étant le braqueur de la banque, l'argent a été saisie.",(_civ getVariable["realname",name _civ])]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
};