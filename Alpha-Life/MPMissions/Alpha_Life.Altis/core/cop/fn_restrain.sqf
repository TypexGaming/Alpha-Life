...
[] spawn
{
	...
	while {true} do
	{
		...
		if(!([west,getPos player,30] call life_fnc_nearUnits) && (player getVariable["restrained",FALSE]) && vehicle player == player) exitWith {
			...
			detach player;
			titleText["Vous avez été détacher automatiquement pour un temps de retenue excessif.","PLAIN"];
			player say3D "cuff";
		};
	};
};

titleText[format["Vous avez êtes menotté par %1",_cop getVariable["realname",name _cop]],"PLAIN"];
player say3D "cuff";
