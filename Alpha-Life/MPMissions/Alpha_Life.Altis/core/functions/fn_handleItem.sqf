								...
								else
							{
									...
									else
								{
									private["_wepItems","_action","_slotTaken"];
										...
										if(_slotTaken) then {
											_action = ["Voulez-vous ajouter cette objet à votre arme ou à votre inventaire? Si vous l'ajoutez à votre arme, l'équipement déjà présent sera remplacer!","Emplacement déjà utiliser","Arme","Inventaire"] call BIS_fnc_guiMessage;
											if(_action) then {
												switch(_type) do {
													case 1: {player addPrimaryWeaponItem _item;};
													case 2: {player addSecondaryWeaponItem _item;};
