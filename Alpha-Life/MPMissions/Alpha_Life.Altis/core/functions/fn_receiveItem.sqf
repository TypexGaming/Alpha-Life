...
if(_diff != (parseNumber _val)) then
{
	...
	{
		hint format["%1 vous a donné %2 mais vous ne pouvez que détenir %3, %4 est donc restitué à son propriétaire.",_from getVariable["realname",name _from],_val,_diff,((parseNumber _val) - _diff)];
	...
	else
{
	if(([true,_item,(parseNumber _val)] call life_fnc_handleInv)) then
	{
		...
		hint format["%1 vous a donné %2 %3",_from getVariable["realname",name _from],_val,_type];
	}
		else
	{
		[[_from,_item,_val,_unit,false],"life_fnc_giveDiff",_from,false] spawn life_fnc_MP;
