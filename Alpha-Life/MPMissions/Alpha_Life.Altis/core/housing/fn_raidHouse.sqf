...
if(isNil {(_house getVariable "house_owner")}) exitWith {hint "Cette maison appartient a personne."};

_uid = (_house getVariable "house_owner") select 0;
if(!([_uid] call life_fnc_isUIDActive)) exitWith {hint "Cette personne n'est pas en ligne, vous ne pouvez pas défoncer sa porte!"};
...
5 cutText ["","PLAIN"];
if(player distance _house > 13) exitWith {life_action_inUse = false; titleText["Vous êtes trop loin de la maison","PLAIN"]};
...
if(_value > 0) then {
	[[0,format["Une maison a été perquisitionné, la Police a retrouvé %1$ de Drogue/Contrebande.",[_value] call life_fnc_numberText]],"life_fnc_broadcast",true,false] spawn life_fnc_MP;
	...
	hint "Rien d'illégale dans cette maison.";
};