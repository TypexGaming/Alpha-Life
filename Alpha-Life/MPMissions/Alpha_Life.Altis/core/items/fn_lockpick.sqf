...
if(_isVehicle && _curTarget in life_vehicles) exitWith {hint "Vous avez déjà les clés de ce véhicule."};
...
if(!isNil "_badDistance") exitWith {titleText["Vous êtes partis trop loin de la cible","PLAIN"]; life_action_inUse = false;};
...
if(!_isVehicle) then {
	...
	if(_dice < 30) then {
		...
		[[0,format["%1 essaye de crocheter un véhicule.",profileName]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
		titleText["Le kit de crochetage a cassé.","PLAIN"];
	};
};