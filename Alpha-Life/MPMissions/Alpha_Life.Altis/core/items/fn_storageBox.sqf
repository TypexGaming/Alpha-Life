...
if(!(_house in life_vehicles)) exitWith {hint "Vous devez être dans votre maison pour placer ceci."};
_containers = _house getVariable["containers",[]];
_houseCfg = [(typeOf _house)] call life_fnc_houseConfig;
if(count _houseCfg == 0) exitWith {}; //What the fuck happened?
if(count _containers >= (_houseCfg select 1)) exitWith {hint "Vous ne pouvez pas mettre plus de coffre de stockage dans cette maison."};
...
if(_pos isEqualTo [0,0,0]) exitWith {hint "Plus d'espace de stockage dans cette maison."};
