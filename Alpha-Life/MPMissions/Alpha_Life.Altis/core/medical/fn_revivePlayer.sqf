...
if(_target getVariable ["Reviving",ObjNull] == player) exitWith {hint "Quelqu'un réanime déja cette personne.";};
if(player distance _target > 5) exitWith {}; //Not close enough.

//Fetch their name so we can shout it.
_targetName = _target getVariable["name","Unknown"];
_title = format["Réanimation %1",_targetName];
...
if(_target getVariable["Revive",FALSE]) exitWith {hint "Cette personne a respawn ou a déja été réanimer."};
...
titleText[format["Vous avez réanimer %1 vous recevez %2$ pour vos services.",_targetName,[(call life_revive_fee)] call life_fnc_numberText],"PLAIN"];
