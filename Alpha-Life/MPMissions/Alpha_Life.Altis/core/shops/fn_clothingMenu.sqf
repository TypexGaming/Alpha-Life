...
//Cop / Civ Pre Check
if((_this select 3) in ["bruce","dive","reb","kart","donator"] && playerSide != civilian) exitWith {hint "Vous devez être un civil pour utiliser ce magasin!"; closeDialog 0;};
if((_this select 3) == "reb" && !license_civ_rebel) exitWith {hint "Vous n'avez pas l'entrainement rebelle."; closeDialog 0;};
if((_this select 3) in ["cop"] && playerSide != west) exitWith {hint "Vous devez être policier pour utiliser ce magasin!"; closeDialog 0;};
if((_this select 3) in ["dive"] && !license_civ_dive) exitWith { hint "Vous avez besoin d'une licence de plongée sous-marine pour utiliser cette boutique!"; closeDialog 0;};
if((_this select 3) in ["donator"] && !license_civ_donator) exitWith { hint "Vous devez être donateur pour utiliser cette boutique"; closeDialog 0;};
