...
if(_veh distance impound_obj < 50) then
{
	...
	{
		_price = _price + 200;
		if(_price > life_atmcash) exitWith {hint "Vous n'avez pas assez d'argent sur ​​vous ou dans votre banque pour récupérer votre véhicule."};
	...
	hint format["Vous avez annulé la mise en fourrière de votre %1 pour %3$",_name,[_price] call life_fnc_numberText];
	detach _veh;
