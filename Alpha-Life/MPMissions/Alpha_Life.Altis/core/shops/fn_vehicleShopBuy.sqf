...
if((lbCurSel 2302) == -1) exitWith {hint "Vous n'avez pas sélectioner un véhicule"};
...
if(life_cash < _basePrice) exitWith {hint format["Vous n'avez pas assez d'argent pour acheter ce véhicule.\n\nMontant manquant: %1$",[_basePrice - life_cash] call life_fnc_numberText];};
if(!([_className] call life_fnc_vehShopLicenses) && _className != "B_MRAP_01_hmg_F") exitWith {hint "Vous n'avez pas la licence requise!"};
...
if(_spawnPoint == "") exitWith {hint "Il y a un véhicule qui bloque actuellement le points de spawn";};
