...
if(_amount > (missionNameSpace getVariable _var)) exitWith {hint "Vous n'avez pas d'objet a vendre!"};

_price = (_price * _amount);
_name = [_var] call life_fnc_vartostr;
if(([false,_type,_amount] call life_fnc_handleInv)) then
{
	hint format["Vous venez de vendre %1 %2 pour %3$",_amount,_name,[_price] call life_fnc_numberText];
