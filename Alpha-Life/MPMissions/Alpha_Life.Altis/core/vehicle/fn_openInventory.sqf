...
if((_vehicle getVariable ["trunk_in_use",false])) exitWith {hint "L'inventaire de ce véhicule est déjà utilisé, une seul personne peux l'utiliser a la fois."};
...
if(_vehicle isKindOf "House_F") then {
	ctrlSetText[3501,format["Stockage Maison - %1",getText(configFile >> "CfgVehicles" >> (typeOf _vehicle) >> "displayName")]];
} else {
	ctrlSetText[3501,format["Coffre Véhicule - %1",getText(configFile >> "CfgVehicles" >> (typeOf _vehicle) >> "displayName")]];
};
...
if(_vehicle isKindOf "House_F" && {count (_vehicle getVariable ["containers",[]]) == 0}) exitWith {hint "Vous devez installer des caisse ou conténaire pour pouvoir ranger vos objets!"; closeDialog 0; _vehicle setVariable["trunk_in_use",false,true];};
if(_veh_data select 0 == -1 && {!(_vehicle isKindOf "House_F")}) exitWith {closeDialog 0; _vehicle setVariable["trunk_in_use",false,true]; hint "Ce véhicule n'est pas en mesure de stocker des objet virtuel.";};
