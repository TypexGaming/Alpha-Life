...
if(_ctrl == "goldbar" && {!(life_trunk_vehicle isKindOf "LandVehicle" OR life_trunk_vehicle isKindOf "House_F")}) exitWith {hint "Tu ne peux mettre les lingots que dans des véhicules terreste!"};
...
{
	if(((_totalWeight select 1) + _itemWeight) > (_totalWeight select 0)) exitWith {hint "Ce véhicule est rempli ou alors vous ne pouvez pas mettre cet objet."};

	if(!([false,_ctrl,_num] call life_fnc_handleInv)) exitWith {hint "Vous ne pouvez pas retirer cet item de votre inventaire pour le mettre dans le véhicule.";};
	_index = [_ctrl,_inv] call fnc_index;
	if(_index == -1) then
	{
		_inv set[count _inv,[_ctrl,_num]];
